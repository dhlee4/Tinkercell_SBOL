...
namespace Tinkercell
{
	...
	void  NodesTree::readTreeFile(const QString& filename)
	{
		...
		for (int i=0; i < keys.size(); ++i)
		{
			...
			if (node && node->parents().isEmpty() && !treeItems.contains(keys[i]))
			{
				families << node;
				parentTreeItems << 0;
			}
		...
		if (families.isEmpty())
		{
			...
			{
				NodeFamily * node = Ontology::nodeFamily(keys[i]);
				if (node && !treeItems.contains(keys[i]))
				{
					if (node->parent() && treeItems.contains(node->parent()->name()))
					{
						families << node;
						parentTreeItems << treeItems[ node->parent()->name() ];
					}
				}
			}
		}
	...
	void NodesTree::updateTheme()
	{
		...
		if (!graphicsItems.isEmpty() && currentNetwork())
			currentNetwork()->push(new ReplaceNodeGraphicsCommand("Theme changed (nodes)", graphicsItems, graphicsFiles));
	}
}
