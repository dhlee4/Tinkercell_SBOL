...
static int isSBOLNamespace(const xmlChar *uri) {
	return (strcmp((char*)uri, NSURL_RDF) == 0 ||
		strcmp((char*)uri, NSURL_RDFS) == 0 ||
		strcmp((char*)uri, NSURL_SO) == 0 ||
		strcmp((char*)uri, NSURL_SBOL) == 0);
}
...
void readSBOLString(Document *destination, const char* sbol_info, int *ret)
{
    DESTINATION = destination;

	// parse
	safeXmlInitParser();
	DOCUMENT = xmlParseDoc(sbol_info);
	...
	if (!isValidSBOL(DOCUMENT)) {
		printf("not a valid SBOL document.\n");
		*ret = isValidSBOL(DOCUMENT);
		return ;
	}
